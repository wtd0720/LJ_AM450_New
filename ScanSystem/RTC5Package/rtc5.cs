...
namespace ScanSystem
{
    ...
    public sealed class Rtc5 : IDisposable
    {
        ...
        public bool Initialize(double kFactor, LaserMode laserMode, string ctbFileName)
        {
            ...
            uint error0 = RTC5Wrap.init_rtc5_dll();
            if (error0 != 0U && error0 != 2U)
            {
                //MessageBox.Show(CtlGetErrMsg(error0));
                return false;
            }
            ...
            ///active high
            RTC5Wrap.n_set_laser_control(this.Index + 1, 0x08);
        ...
        public bool CtlFrequency(double frequency, double pulseWidth, double power)
        {
            if (this.CtlGetStatus(RtcStatus.Busy))
                return false;
            ...
            RTC5Wrap.n_set_laser_timing(this.Index + 1,
                ...
                0,
                0);
            RTC5Wrap.n_write_da_1_list(this.Index + 1, (uint)power * 1024 / 100);
        ...
        public bool ListFrequency(double frequency, double pulseWidth ,double power)
        {
